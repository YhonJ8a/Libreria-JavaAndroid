...
public class DbConeccion extends DbHelper{
    ...
    public long insertarNuevoLibro(String nombre, String autor, String cantidad, String descripcion, String foto, String url) {
    ...
    public int codUsuario(){
        ...
        cursorUsers= db.rawQuery("select id_usuario from "+ TABLE_USUARIOS+" WHERE correo = '"+ sp.getUsuarioActivo()+"'", null);
    ...
    public ArrayList<Libros> mostrarLibrosDisponibles(){
        ...
        cursorContactos = db.rawQuery("select * from "+TABLE_LIBRO+" where cantidad > 0 ", null);

        if(cursorContactos.moveToFirst()){
            do {
                libros = new Libros();
                libros.setCod_libro(cursorContactos.getInt(0));
                libros.setNom_libro(cursorContactos.getString(1));
                libros.setAutor(cursorContactos.getString(2));
                libros.setCantidad(cursorContactos.getInt(3));
                libros.setDescripcion(cursorContactos.getString(4));
                libros.setFoto(cursorContactos.getString(5));
                libros.setUrl(cursorContactos.getString(6));
                listaContactos.add(libros);
            }while (cursorContactos.moveToNext());
        }else {
            libros = new Libros();
    ...
    public String nombreUsuario(){
        ...
        cursorUsers= db.rawQuery("select nombre from "+ TABLE_USUARIOS+" WHERE correo = '"+ sp.getUsuarioActivo()+"'", null);
    ...
    public boolean editarLibro(int id,String nombre, String autor , int cantidad, String url, String foto, String descripcion) {
        ...
        try {
            db.execSQL("UPDATE "+TABLE_LIBRO+
                    " SET nom_libro = '"+ nombre+
                    "', autor = '"+autor+
                    "', cantidad = '"+ cantidad+
                    "', url = '"+ url+
                    "', foto = '"+ foto+
